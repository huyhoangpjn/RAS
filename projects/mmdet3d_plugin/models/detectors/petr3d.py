...
class Petr3D(MVXTwoStageDetector):
    ...
    def extract_img_feat(self, img, len_queue=1, training_mode=False):
        ...
        if self.training or training_mode:
            img_feats_reshaped = img_feats[self.position_level].view(B, len_queue, int(BN/B / len_queue), C, H, W)# stack many frames into a batch, reason why training with window cost a lot of memory 
        else:
    ...
    def forward_roi_head(self, location, **data):
        # Enable the focal head in inferece step
    ...
    def forward_pts_train(self,
        ...
        if return_losses:
            ...
            losses = self.pts_bbox_head.loss(*loss_inputs, img_metas=img_metas, **data)
