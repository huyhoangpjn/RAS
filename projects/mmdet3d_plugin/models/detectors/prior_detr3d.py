...
# Windown training should be deprecated

@DETECTORS.register_module()
class PriorDetr3D(MVXTwoStageDetector):
    """Detr3D."""
    ...
    def __init__(self,
                 ...
                 num_frame_losses=2,
                 multi_levels=[0,1,2], # Temporary adaptation for 2D detector
                 train_2d = False,
                 single_test=False,
                 pretrained=None):
        super(PriorDetr3D, self).__init__(pts_voxel_layer, pts_voxel_encoder,
        ...
        # This allows only 1 detector to be trained at a time (either 2D or 3D)
        # 2D training means only allow the backprop on 2D head - Deprecated
        # if train_2d:
        #     # freeze backbone in config file
        #     # freeze neck
    ...
    def extract_img_feat(self, img, len_queue=1, training_mode=False):
        ...
        # Currently only get 1 level (StreamPETR)
        B = img.size(0)
        ...
        multi_lv_feats = []
        for level in self.multi_levels:
            BN, C, H, W = img_feats[level].size()
            if self.training or training_mode:
                multi_lv_feats.append(img_feats[level].view(B, len_queue, BN//B//len_queue, C, H, W)) # always use seq training mode; len_queue = 1
            else: multi_lv_feats.append(img_feats[level].view(B, BN//B//len_queue, C, H, W))
        return multi_lv_feats

    @auto_fp16(apply_to=('img', 'multi_lv_feats'), out_fp32=True)
    def extract_feat(self, img, T, training_mode=False):
        """Extract features from images and points."""
        multi_lv_feats = self.extract_img_feat(img, T, training_mode)  
        return multi_lv_feats
    ...
    def obtain_history_memory(self,
        ...
        for i in range(T):
            ...
            return_losses = False
            data_t = dict()
            data_t['multi_lv_feats'] = []
            for key in data:
                if key != 'multi_lv_feats':
                    data_t[key] = data[key][:, i]
                else:
                    for lv in range(len(data['multi_lv_feats'])):
                        data_t['multi_lv_feats'].append(data['multi_lv_feats'][lv][:, i])
    ...
    def forward_pts_train(self,
        ...
        if not requires_grad:
            ...
            with torch.no_grad():
                ...
                outs_bboxes = self.img_roi_head.get_bboxes(outs_roi, threshold_score=self.img_roi_head.threshold_score)
                outs = self.pts_bbox_head(img_metas, outs_bboxes, **data)
            self.train()

        else:
            outs_roi = self.forward_roi_head(**data)
            outs_bboxes = self.img_roi_head.get_bboxes(outs_roi, threshold_score=self.img_roi_head.threshold_score)
            # Use this if self.use_soft_label is True
            # gt_conf2d = self.img_roi_head.get_highest_conf_for_gt_2d(outs_roi, gt_bboxes, gt_labels)
            # outs_bboxes.update(gt_conf2d=gt_conf2d)
            outs = self.pts_bbox_head(img_metas, outs_bboxes, **data)
    ...
    def forward_train(self,
        ...
        multi_lv_feats = self.extract_feat(rec_img, self.num_frame_backbone_grads, True) # Should specify training_mode here
        ...
        if T-self.num_frame_backbone_grads > 0:
            ...
            self.train()
            data['multi_lv_feats'] = [torch.cat([prev_img_feats[i], multi_lv_feats[i]], dim=1) for i in range(len(self.multi_levels))]
        else:
            data['multi_lv_feats'] = multi_lv_feats
        ...
        for var, name in [(img_metas, 'img_metas')]:
            if not isinstance(var, list):
                raise TypeError('{} must be a list, but got {}'.format(
                    name, type(var)))
        for key in data:
            # Fix here if multi_lv_feat getting errors
            if key != 'img':
    ...
    def simple_test(self, img_metas, **data):
        ...
        data['multi_lv_feats'] = self.extract_img_feat(data['img'], 1)
