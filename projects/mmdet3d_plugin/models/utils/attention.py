...
class FlashAttention(nn.Module):
    ...
    def forward(self, q, kv, 
        ...
        assert q.shape[0] == kv.shape[0] and q.shape[-2] == kv.shape[-2] and q.shape[-1] == kv.shape[-1] # check nb batch, head, dim 
...
class FlashMHA(nn.Module):
    ...
    def forward(self, q, k, v, key_padding_mask=None):
        ...
        return self.out_proj(rearrange(context, 'b s h d -> b s (h d)')), #  (q,k), attn_weights q,k for attn mask visualization, attn_weights here = None due to different computational mechanism. While training return None to save memory 
