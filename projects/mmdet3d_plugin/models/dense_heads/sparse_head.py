...
class SparseHead(AnchorFreeHead):
    ...
    def __init__(self,
                 ...
                 distance_temp=2.0,
                 include_ori=False,
                 use_group_detr=False,
                 num_group=1,
                 group_loss_weight=0.25,
        ...
        self.use_group_detr = use_group_detr
        ...
        self.extra_code_weights = nn.Parameter(torch.tensor(
            self.extra_code_weights[:self.code_size]), requires_grad=False)
    ...
    def _init_layers(self):
        ...
        if self.use_group_detr:
            self.group_reference_points = [nn.Embedding(900, 3) for _ in range(self.num_group)]
    ...
    def prepare_for_dn(self, batch_size, reference_points, img_metas):
        if self.training and self.with_dn:
            ...
            # Calculate number of group queries (only during training with group_detr)
            num_group_queries = self.num_group * (self.num_query + self.num_propagated) if self.use_group_detr else 0
            group_query_size = self.num_query + self.num_propagated  # size of each group
            
            tgt_size = pad_size + self.num_query + num_group_queries
            attn_mask = torch.ones(tgt_size, tgt_size).to(reference_points.device) < 0
            # match query cannot see the reconstruct (DN queries)
            ...
            # Handle group DETR masking: each group can only attend to itself
            if self.use_group_detr and num_group_queries > 0:
                # Group queries start after pad_size + num_query (normal queries)
                group_start_idx = pad_size + self.num_query
                
                for g in range(self.num_group):
                    group_g_start = group_start_idx + g * group_query_size
                    group_g_end = group_g_start + group_query_size
                    
                    # Group g cannot attend to DN queries
                    attn_mask[group_g_start:group_g_end, :pad_size] = True
                    
                    # Group g cannot attend to normal queries
                    attn_mask[group_g_start:group_g_end, pad_size:group_start_idx] = True
                    
                    # Group g cannot attend to other groups
                    for other_g in range(self.num_group):
                        if other_g != g:
                            other_g_start = group_start_idx + other_g * group_query_size
                            other_g_end = other_g_start + group_query_size
                            attn_mask[group_g_start:group_g_end, other_g_start:other_g_end] = True
                    
                    # Normal queries cannot attend to group g queries
                    attn_mask[pad_size:group_start_idx, group_g_start:group_g_end] = True
             
            # update dn mask for temporal modeling
            query_size = pad_size + self.num_query + num_group_queries + self.num_propagated
            tgt_size = pad_size + self.num_query + num_group_queries + self.memory_len
            ...
            # Handle group DETR temporal masking: groups cannot attend to temporal memory
            # if self.use_group_detr and num_group_queries > 0:
            #     group_start_idx = pad_size + self.num_query
            #     group_end_idx = group_start_idx + num_group_queries
            #     temporal_start_idx = pad_size + self.num_query + num_group_queries
                
            #     # Group queries cannot attend to temporal memory (keys beyond the initial tgt_size)
            #     temporal_attn_mask[group_start_idx:group_end_idx, attn_mask.size(1):] = True
                
            #     # Temporal queries (propagated) cannot attend to group queries
            #     temporal_attn_mask[group_end_idx:, group_start_idx:group_end_idx] = True
    ...
    def post_update_memory(self, data, rec_ego_pose, all_cls_scores, all_bbox_preds, outs_dec, mask_dict):
        if self.training and mask_dict and mask_dict['pad_size'] > 0:
            ...
            # Remove group queries if using group DETR during training
            # Structure after removing DN padding: [normal_queries, group_queries, temporal_queries]
            # We only want [normal_queries, temporal_queries] for memory update
            if self.use_group_detr:
                num_group_queries = self.num_group * (self.num_query + self.num_propagated)
                # Keep normal queries (first num_query) and temporal queries (last num_propagated)
                normal_end_idx = self.num_query
                temporal_start_idx = self.num_query + num_group_queries
                
                rec_reference_points = torch.cat([rec_reference_points[:, :normal_end_idx], 
                                                   rec_reference_points[:, temporal_start_idx:]], dim=1)
                rec_velo = torch.cat([rec_velo[:, :normal_end_idx], 
                                      rec_velo[:, temporal_start_idx:]], dim=1)
                rec_memory = torch.cat([rec_memory[:, :normal_end_idx], 
                                        rec_memory[:, temporal_start_idx:]], dim=1)
                rec_score = torch.cat([rec_score[:, :normal_end_idx], 
                                       rec_score[:, temporal_start_idx:]], dim=1)
                rec_timestamp = torch.cat([rec_timestamp[:, :normal_end_idx], 
                                           rec_timestamp[:, temporal_start_idx:]], dim=1)
                rec_ego_pose = torch.cat([rec_ego_pose[:, :normal_end_idx], 
                                          rec_ego_pose[:, temporal_start_idx:]], dim=1)
        else:
    ...
    def forward(self, img_metas, **data):
        ...
        if self.use_group_detr and self.training:
            reference_points = torch.cat([reference_points, torch.cat([grp_ref.weight for grp_ref in self.group_reference_points], dim=0).to(reference_points.device)], dim=0).to(reference_points.device)
        ...
        if self.use_ray_o2m:
            outputs_classes_o2m = []
            for lvl in range(outs_dec.shape[0]):
                outputs_class_o2m = self.cls_branches_o2m[lvl](outs_dec[lvl])
    ...
    def _get_target_single(self,
        ...
        if self.use_ray_o2m:
            ...
            if self.gt_balancing:
                ...
                if len(all_pos_inds) > 0:
                    ...
                    for i in range(len(unique_gt_inds)):
                        mask = (inverse_indices == i)
                        group_neg_distances = neg_distances[mask]
                        if group_neg_distances.numel() > 1:
                            group_softmax = torch.softmax(group_neg_distances, dim=0)
                            softmax_weights[mask] = group_softmax
                        else:
                            # Single prediction for this GT, weight = 1.0
                            softmax_weights[mask] = 1.0
        ...
        if sampling_result.num_gts > 0:
            ...
            bbox_weights[pos_inds] = 1.0 if (not self.use_ray_o2m or self.gt_balancing) else box_weights_o2o.unsqueeze(-1).repeat(1, 10)
        ...
        return (labels, label_weights, bbox_targets, bbox_weights, 
                pos_inds, neg_inds) if not self.use_ray_o2m else \
            (labels, label_weights, bbox_targets, bbox_weights,
            pos_inds, neg_inds, labels_o2m, label_weights_o2m, 
            pos_inds_o2m, neg_inds_o2m)
    ...
    def get_targets(self,
        ...
        return (labels_list, label_weights_list, bbox_targets_list,
                bbox_weights_list, num_total_pos, num_total_neg) if not self.use_ray_o2m else \
            (labels_list, label_weights_list, bbox_targets_list,
    ...
    def loss_single(self,
        ...
        if self.use_ray_o2m:
            ...
            # classification loss for o2m
            cls_scores_o2m = cls_scores_o2m.reshape(-1, self.cls_out_channels)
            cls_avg_factor_o2m = num_total_o2m_pos * 1.0 + \
                num_total_o2m_neg * self.bg_cls_weight
                # weighted: label_weights_o2m.sum().item()

            if self.sync_cls_avg_factor:
                cls_avg_factor_o2m = reduce_mean(
                    cls_scores_o2m.new_tensor([cls_avg_factor_o2m]))
        ...
        # Use provided code_weights or default to self.code_weights
        used_code_weights = code_weights if code_weights is not None else self.code_weights
        bbox_weights = bbox_weights * used_code_weights
        ...
        return (loss_cls, loss_bbox) if not self.use_ray_o2m else \
            (loss_cls, loss_bbox, loss_cls_o2m)
    ...
    def loss(self,
        ...
        # Handle group DETR: split predictions into main queries and group queries
        # Query order after removing DN: [Normal queries | Group queries | Temporal queries]
        # Group queries size = num_group * (num_query + num_propagated)
        if self.use_group_detr and self.training:
            num_group_queries = self.num_group * (self.num_query + self.num_propagated)
            group_query_size = self.num_query + self.num_propagated
            
            # Split: main queries = [Normal queries | Temporal queries]
            # all_cls_scores shape: [nb_dec, bs, total_queries, cls_out_channels]
            # Total queries = num_query + num_group_queries + num_propagated
            main_cls_scores = torch.cat([
                all_cls_scores[:, :, :self.num_query, :],  # Normal queries
                all_cls_scores[:, :, self.num_query + num_group_queries:, :]  # Temporal queries
            ], dim=2)
            main_bbox_preds = torch.cat([
                all_bbox_preds[:, :, :self.num_query, :],
                all_bbox_preds[:, :, self.num_query + num_group_queries:, :]
            ], dim=2)
            
            # Extract group queries
            group_cls_scores = all_cls_scores[:, :, self.num_query:self.num_query + num_group_queries, :]
            group_bbox_preds = all_bbox_preds[:, :, self.num_query:self.num_query + num_group_queries, :]
        else:
            main_cls_scores = all_cls_scores
            main_bbox_preds = all_bbox_preds
        ...
        if self.use_ray_o2m:
            ...
            # Also split o2m scores for group DETR
            if self.use_group_detr and self.training:
                main_cls_scores_o2m = torch.cat([
                    all_cls_scores_o2m[:, :, :self.num_query, :],
                    all_cls_scores_o2m[:, :, self.num_query + num_group_queries:, :]
                ], dim=2)
            else:
                main_cls_scores_o2m = all_cls_scores_o2m
            ...
            all_pseudo_gt_bboxes_list = [
                pseudo_gt_bboxes_list for _ in range(num_dec_layers)
            ]
            all_pseudo_gt_labels_list = [
                pseudo_gt_labels_list for _ in range(num_dec_layers)
            ]
            all_pseudo_corr_idx_list = [
                pseudo_corr_idx_list for _ in range(num_dec_layers)
            ]
            # img_metas = [img_metas for _ in range(num_dec_layers)]
            # lidar2imgs = [data['lidar2img'] for _ in range(num_dec_layers)]
            (losses_cls, losses_bbox, losses_cls_o2m) = multi_apply(
                self.loss_single, main_cls_scores, main_bbox_preds,
                all_gt_bboxes_list, all_gt_labels_list, 
                all_gt_bboxes_ignore_list, main_cls_scores_o2m,
            ...
            num_dec_layer = 0
            for loss_cls_o2m_i in losses_cls_o2m[:-1]:
                loss_dict[f'd{num_dec_layer}.loss_cls_o2m'] = loss_cls_o2m_i
                num_dec_layer += 1
        else:
        ...
        # Group DETR loss: each group supervised separately with duplicated GTs
        if self.use_group_detr and self.training:
            # Prepare extra_code_weights list for multi_apply (one per decoder layer)
            all_extra_code_weights = [self.extra_code_weights for _ in range(num_dec_layers)]
            
            for g in range(self.num_group):
                # Extract predictions for group g
                g_start = g * group_query_size
                g_end = g_start + group_query_size
                group_g_cls_scores = group_cls_scores[:, :, g_start:g_end, :]
                group_g_bbox_preds = group_bbox_preds[:, :, g_start:g_end, :]
                
                # Compute loss for this group with the same GTs (duplicated supervision)
                # Use extra_code_weights for group queries
                if self.use_ray_o2m:
                    group_g_cls_scores_o2m = all_cls_scores_o2m[:, :, self.num_query + g_start:self.num_query + g_end, :]
                    (group_losses_cls, group_losses_bbox, group_losses_cls_o2m) = multi_apply(
                        self.loss_single, group_g_cls_scores, group_g_bbox_preds,
                        all_gt_bboxes_list, all_gt_labels_list, 
                        all_gt_bboxes_ignore_list, group_g_cls_scores_o2m,
                        all_pseudo_gt_bboxes_list, all_pseudo_gt_labels_list,
                        all_pseudo_corr_idx_list, all_extra_code_weights)
                    
                    loss_dict[f'group{g}.loss_cls_o2m'] = group_losses_cls_o2m[-1]
                    num_dec_layer = 0
                    for loss_cls_o2m_i in group_losses_cls_o2m[:-1]:
                        loss_dict[f'd{num_dec_layer}.group{g}.loss_cls_o2m'] = loss_cls_o2m_i
                        num_dec_layer += 1
                else:
                    group_losses_cls, group_losses_bbox = multi_apply(
                        self.loss_single, group_g_cls_scores, group_g_bbox_preds,
                        all_gt_bboxes_list, all_gt_labels_list, 
                        all_gt_bboxes_ignore_list, [None] * num_dec_layers,
                        [None] * num_dec_layers, [None] * num_dec_layers,
                        [None] * num_dec_layers, all_extra_code_weights)
                
                # Add group losses to loss_dict
                loss_dict[f'group{g}.loss_cls'] = group_losses_cls[-1]
                loss_dict[f'group{g}.loss_bbox'] = group_losses_bbox[-1]
                
                # Loss from other decoder layers for this group
                num_dec_layer = 0
                for loss_cls_i, loss_bbox_i in zip(group_losses_cls[:-1],
                                                   group_losses_bbox[:-1]):
                    loss_dict[f'd{num_dec_layer}.group{g}.loss_cls'] = loss_cls_i*self.group_loss_weight
                    loss_dict[f'd{num_dec_layer}.group{g}.loss_bbox'] = loss_bbox_i*self.group_loss_weight
                    num_dec_layer += 1
