...
class FocalHead(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if self.with_depth:
            self.ddn_loss = DDNLoss(depthnet_config)
    ...
    def _init_layers(self):
        ...
        if self.with_depth:
            self.depthnet = SimpleDepthNet(self.depthnet_config, False, 1)
    ...
    def forward(self, location, **data):
        ...
        if self.with_depth and self.training:
            depth_logits = self.depthnet(x)
            depth_preds = depth_logits.softmax(dim=1)
            outs.update(depth_logits = depth_logits, depth_preds = depth_preds)
