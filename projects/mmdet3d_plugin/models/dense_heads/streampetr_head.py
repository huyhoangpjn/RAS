...
from projects.mmdet3d_plugin.models.utils.misc import MLN, topk_gather, transform_reference_points, memory_refresh, SELayer_Linear, ray_eq_generator, ray_adaptive_generator
from projects.mmdet3d_plugin.models.utils.score_functions import compute_ro_diou, compute_ro_diou_pairwise, inbev_score
...
class StreamPETRHead(AnchorFreeHead):
    ...
    def __init__(self,
                 ...
                 scoring_method='inbev_score',
                 cost_ioufunc='compute_ro_diou',
                 loss_ioufunc='compute_ro_diou',
        ...
        if self.use_soft_label_v1:
            ...
            self.scoring_method = scoring_methods.get(scoring_method, inbev_score)

            ioufunc = {
                'compute_ro_diou': compute_ro_diou
            }

            self.cost_ioufunc = ioufunc.get(cost_ioufunc, compute_ro_diou)
            self.loss_ioufunc = ioufunc.get(loss_ioufunc, compute_ro_diou)
        ...
        self.gt_balancing = gt_balancing # Need a method to balance the number of examples per GT
    ...
    def post_update_memory(self, data, rec_ego_pose, all_cls_scores, all_bbox_preds, outs_dec, mask_dict):
        if self.training and mask_dict and mask_dict['pad_size'] > 0:
            ...
            rec_velo = all_bbox_preds[:, :, mask_dict['pad_size']:, -2:][-1] # if cls in the end, why get velo = last 2 elements?
    ...
    def position_embeding(self, data, memory_centers, topk_indexes, img_metas): # wrong in expand the intrinsic cam
        eps = 1e-5
        BN, H, W, _ = memory_centers.shape
        B = data['intrinsics'].size(0)

        intrinsic = torch.stack([data['intrinsics'][..., 0, 0], data['intrinsics'][..., 1, 1]], dim=-1) # size intrinsic B,N,...
    ...
    def prepare_for_dn(self, batch_size, reference_points, img_metas):
        ...
        if self.training and self.with_dn:
            ...
            # Since all batches have the same init global points
    ...
    def forward(self, memory_center, img_metas, topk_indexes=None,  **data):
        ...
        Returns:
            ...
            all_bbox_preds (Tensor): Sigmoid outputs from the regression \
                ...
                Shape [nb_dec, bs, num_query, 9]. size query = 10m them cls o cuoi nua
        """
        ...
        # Pick top k feats among all past feats. Dont worry, prepare_location ensures right indexing of all past feats. But anyways, this only take 1 past frame in sequence training
        # This one if topk_indexes + window training -> 100% error
        memory = topk_gather(memory, topk_indexes)

        # pos_embed: 3d positional encoding of 2D feats != positional encoding of queries
        ...
        ori_query_pos = reference_points
    ...
    def _get_target_single(self,
        ...
        if self.use_soft_label_v1:
            if gt_bboxes.numel():
                ...
                if self.scoring_method.__name__ == 'inbev_score':
                    gt_bboxes_3d = gt_bboxes.clone()
                    bbox_pred_3d = denormalize_bbox(bbox_pred.detach(), None)
                else: raise NotImplementedError('Only inbev_score is supported for now')
                # calculate positional scores
                pos_scores_cost = self.scoring_method(
                    bbox_pred_3d, gt_bboxes_3d, gamma=self.pos_score_for_cost_power, 
                    ioufunc=self.cost_ioufunc, rescale_strategy=2,)
                
                pos_scores_loss = self.scoring_method(
                    bbox_pred_3d, gt_bboxes_3d, gamma=self.pos_score_for_loss_power, 
                    ioufunc=self.loss_ioufunc, rescale_strategy=0)
        ...
        if self.use_ray_o2m:
            ...
            # maybe co the quay ve gen cho may sample ma bi o xa
            ...
            if self.gt_balancing:
                ...
                # Count number of matched predictions for each GT
                unique_gt_inds, counts = torch.unique(all_pos_gt_inds, return_counts=True)
                
                # Create weight tensor for all GTs (inverse of count)
                gt_weights = torch.zeros(num_gt, dtype=torch.float, device=pos_inds_o2m.device)
                gt_weights[unique_gt_inds] = 1.0 / counts.float()
                
                # Calculate weights for pos_inds_o2m and pos_inds in batch
                box_weights_o2m = gt_weights[pos_gt_inds_o2m]
                box_weights_o2o = gt_weights[sampling_result.pos_assigned_gt_inds]
        ...
        if self.use_ray_o2m:
            (labels_list, label_weights_list, bbox_targets_list,
             ...
             labels_o2m_list, label_weights_o2m_list, pos_inds_o2m_list, neg_inds_o2m_list) = multi_apply(
                ...
                cls_scores_o2m, img_metas, lidar2imgs,)
                # pseudo_gt_bboxes_list, pseudo_gt_labels_list,
    ...
    def dn_loss_single(self,
        ...
        if self.use_soft_label_v1:
            ...
            # denormalize_preds = denormalize_bbox(bbox_preds.detach(), self.pc_range)
            # denormalize_known_bboxs = denormalize_bbox(known_bboxs, self.pc_range)
            # soft_labels = compute_ro_diou_pairwise(denormalize_preds, denormalize_known_bboxs) ** self.pos_score_for_loss_power
            # # Rescale following TOOD
            # max_score = soft_labels.max()
            # if max_score > 0:
            #     soft_labels = soft_labels / max_score
            soft_labels = soft_labels.new_ones(known_labels.shape)
            known_labels = (known_labels.long(), soft_labels)
        else:
    ...
    def loss(self,
        ...
        if self.use_ray_o2m:
            ...
            (losses_cls, losses_bbox, losses_cls_o2m) = multi_apply(
                ...
                all_gt_bboxes_ignore_list, all_cls_scores_o2m, img_metas, lidar2imgs,)
